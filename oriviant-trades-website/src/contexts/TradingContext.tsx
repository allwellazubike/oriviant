...
export const TradingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to trigger price flash safely without state updater collision
  ...
  const setActiveCoinSymbol = useCallback((symbol: string) => {
    setActiveSymbol((prev) => (prev === symbol ? prev : symbol));
  }, []);
  ...
  const toggleCategoryFeed = useCallback((cat: keyof MarketCategoryConfig) => {
    setCategoryConfig((prev) => ({ ...prev, [cat]: !prev[cat] }));
  }, []);

  const manualRefreshFeed = useCallback(() => {
    fetchBinanceCryptoTickers();
    fetchForexRates();
  }, [categoryConfig]);
  ...
  }, [isDemoMode, addLedgerEntry]);

  const cancelOrder = useCallback((orderId: string) => {
    setRawOpenOrders((prev) => prev.filter((o) => o.id !== orderId));
  }, []);

  const openFuturesPosition = useCallback((pos: {
    pair: string;
    side: PositionSide;
    leverage: number;
    marginMode: MarginMode;
  ...
  }, [rawPositions, addLedgerEntry]);

  const reversePosition = useCallback((positionId: string) => {
