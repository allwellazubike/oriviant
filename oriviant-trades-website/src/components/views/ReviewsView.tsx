  ...
  const filteredReviews = useMemo(() => {
    return MOCK_PLATFORM_REVIEWS.filter((rev) => {
      ...
      if (selectedCategory !== 'All') {
        const normSel = selectedCategory.toLowerCase().replace(/s$/, '');
        const normRev = rev.category.toLowerCase().replace(/s$/, '');
        if (normSel !== normRev) return false;
      }
      ...
      const catStr = rev.category as string;
      if ((selectedFilter === 'Deposits' || selectedFilter === 'Deposit') && catStr !== 'Deposits' && catStr !== 'Deposit') return false;
      if ((selectedFilter === 'Withdrawals' || selectedFilter === 'Withdrawal') && catStr !== 'Withdrawals' && catStr !== 'Withdrawal') return false;
