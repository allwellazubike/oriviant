    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-xl rounded-3xl bg-app-card border border-app shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          {submittedRecord ? (
            /* Deposit Submitted Success View */
                  ...
                  >
                    <div className="space-y-1">
                      <div className="text-xs font-extrabold text-app flex items-center gap-2">
                        <span>{net.name}</span>
                        {net.memoOrTagRequired && (
                          <span className="px-2 py-0.5 text-[9px] font-bold rounded-full bg-amber-500/20 text-amber-500">
                            MEMO REQUIRED
                          </span>
                        )}
                      </div>
            ...
            <div className="space-y-5">
              ...
              <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-500 text-xs space-y-1">
                ...
                {currentNetworkInfo?.memoOrTagRequired && (
                  <p className="text-[11px] font-bold text-amber-400 underline pt-1">
                    ⚠️ Destination Tag/MEMO is required for this coin. Omitting it will result in lost deposits.
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center bg-app-sub/40 p-4 rounded-2xl border border-app">
                <div className="sm:col-span-1 flex flex-col items-center justify-center p-2 bg-white rounded-xl shadow-inner">
                  <img 
                    src={generateQrUrl(currentNetworkInfo?.depositAddress || '')} 
                    alt="Deposit QR" 
                ...
                <div className="sm:col-span-2 space-y-3">
                  <div>
                    <label className="block text-[10px] font-bold text-app-sec uppercase mb-1">
                      Deposit Address
                    </label>
                    <div className="p-3 rounded-xl bg-app-card border border-app flex items-center justify-between gap-2">
                      <span className="text-xs font-mono font-bold text-app break-all select-all">
                        {currentNetworkInfo?.depositAddress}
                      </span>
                      <button
                        onClick={() => handleCopy(currentNetworkInfo?.depositAddress || '')}
                        className="p-2 rounded-lg bg-accent/10 hover:bg-accent/20 text-accent transition-colors shrink-0"
                        title="Copy Address"
                      >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleCopy(currentNetworkInfo?.depositAddress || '')}
                      className="flex-1 py-2 rounded-xl bg-app-sub hover:bg-app-card text-app font-bold text-xs border border-app transition-all flex items-center justify-center gap-1.5"
                    >
                      <Copy className="w-3.5 h-3.5 text-accent" />
                      <span>{copied ? 'Copied!' : 'Copy Address'}</span>
                    </button>
                    <button
                      onClick={() => handleCopy(`https://oriviant.io/deposit?asset=${selectedSymbol}&address=${currentNetworkInfo?.depositAddress}`)}
                      className="py-2 px-3 rounded-xl bg-app-sub hover:bg-app-card text-app-sec hover:text-app font-bold text-xs border border-app transition-all flex items-center gap-1"
                    >
