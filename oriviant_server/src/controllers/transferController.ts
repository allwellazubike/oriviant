...
export const handleTransfer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // FIX: Explicitly include from_wallet and to_wallet keys from the frontend payload
    const asset = req.body.asset || req.body.symbol || req.body.assetSymbol;
    const amount = Number(req.body.amount);
    const from_type = req.body.from_type || req.body.from_wallet || req.body.fromWallet || req.body.from || 'spot';
    const to_type = req.body.to_type || req.body.to_wallet || req.body.toWallet || req.body.to || 'futures';
    ...
    // 3. Check or Create destination sub-wallet
