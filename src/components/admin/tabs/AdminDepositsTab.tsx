...
export const AdminDepositsTab: React.FC = () => {
  ...
  const fetchDeposits = async () => {
    setIsLoading(true);
    try {
      const res = await adminApi.getDeposits('ALL'); // Fetch all statuses to populate the history
      if (res.success && res.data) {
        const mapped: DepositRecord[] = res.data.map((d: any) => ({
          id: d.id.toString(),
          user: d.nickname || 'Unknown User',
          email: d.email || 'N/A',
          asset: d.asset,
          amount: Number(d.amount_expected),
          usdValue: Number(d.amount_expected), // Assuming 1:1 for simplicity without live oracle
          network: d.network || 'Unknown',
          txHash: d.tx_hash || 'N/A',
          depositAddress: d.deposit_address || 'N/A',
          status: d.status === 'APPROVED' ? 'Successful' : d.status === 'DENIED' ? 'Rejected' : 'Pending',
          date: new Date(d.created_at).toISOString().split('T')[0],
          time: new Date(d.created_at).toISOString().split('T')[1].substring(0, 8),
  ...
  const handleUpdateStatus = async (id: string, newStatus: DepositRecord['status'], note?: string) => {
    ...
    try {
      if (newStatus === 'Successful') {
        ...
        if (res.success) {
          ...
          await fetchDeposits(); // Refresh list to reflect DB changes
        } else {
          ...
          <div className="w-full max-w-md bg-app-card border border-app rounded-3xl p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-app-sec border border-app rounded-xl p-3 text-xs text-app focus:outline-none focus:border-amber-500 disabled:opacity-50"
              />
            </div>
            ...
            <div className="pt-3 border-t border-app flex items-center justify-end gap-2">
              <button
                ...
                disabled={isActionLoading}
                className="px-4 py-2 rounded-xl bg-app-sec text-app-sec hover:text-app font-bold text-xs disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isActionLoading}
                className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-white font-extrabold text-xs shadow-md disabled:opacity-50 ${
